...
- **图片悬浮窗** - 独立图片悬浮窗，内置弹药类型后坐力参考图，支持左右切换浏览，也可自选图片（欢迎提交 PR 更新后坐力图片或添加其他游戏的图片）
- **三档采样性能** - 低(50Hz)/中(1000Hz)/高(8000Hz) 三档可选，按需平衡精度与CPU占用
...
- Windows 7 SP1 / Windows 10 / Windows 11 (x64)
- 无需额外安装 .NET 运行时（安装包已包含）
- USB 或蓝牙连接的手柄
...
前往 [Releases](../../releases) 页面下载最新的 `CCWaterControllerPlayer-Setup-vX.X.X.exe` 安装包，运行安装即可。
